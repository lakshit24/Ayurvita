...
public class Database extends SQLiteOpenHelper {
    ...
    public int login(String username,String Password){
        ...
        str[0]=username;
        str[1]=Password;
        SQLiteDatabase dbLogin=getReadableDatabase();
        Cursor c = dbLogin.rawQuery("select * from users where username=? and password=?",str);
        if (c.moveToFirst()){
            result=1;
        }
        return result;
    ...
    public int checkCart(String username,String product){
        ...
        str[0]=username;
        str[1]=product;
        SQLiteDatabase dbcart=getReadableDatabase();
        Cursor c = dbcart.rawQuery("select * from cart where username=? and product=?",str);
        if (c.moveToFirst()){
            result=1;
        }
        dbcart.close();
